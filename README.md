...
user_schema.create_index({name: 1, type: -1});
...
* **Document instance returns**: data-returning methods return document instances with top-level metadata (`id`, `created_at`, `updated_at`), and `.to_json()` / `.to_object()` for plain-object snapshots.
