...
// returns: { id, user_name, age, type, created_at, updated_at }

// 7. Find a document
const found_user = await User.find_one({
	user_name: 'john'
}).exec();
// returns: { id, user_name, age, type, created_at, updated_at } or null
```
...
* **Flat metadata returns**: data-returning methods include `id`, `created_at`, and `updated_at` at the top level.
