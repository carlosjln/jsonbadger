...
describe('Query operators', function () {
	...
	test('maps native and compat $exists operators to SQL fragments', function () {
		const native_exists_parameter_state = create_parameter_state();
		const compat_exists_parameter_state = create_parameter_state();

		expect(exists_native_operator('data -> \'profile\'', true, native_exists_parameter_state, {
			data_column_reference: 'data',
			path_value: 'profile'
		})).toBe('data @? $1::jsonpath');
		expect(native_exists_parameter_state.params).toEqual(['$.profile']);

		expect(exists_compat_operator('data -> \'profile\'', false, compat_exists_parameter_state)).toBe('(data -> \'profile\') IS NULL');
		expect(compat_exists_parameter_state.params).toEqual([]);
	});
