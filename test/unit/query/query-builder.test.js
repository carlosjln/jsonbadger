...
describe('QueryBuilder.exec read behavior', function () {
	...
	test('constructor requires model.options', function () {
		expect(function () {
			return new QueryBuilder({schema: {}}, 'find', {}, null);
		}).toThrow('QueryBuilder requires model.options');

		expect(function () {
			return new QueryBuilder({}, 'find', {}, null);
		}).toThrow('QueryBuilder requires model.schema');
	});

	test('constructor and chain methods normalize falsy inputs to their defaults', function () {
		const query_builder = new QueryBuilder(create_model_stub(), 'find', null, undefined);
		...
		expect(query_builder.sort_filter).toBeNull();
	});
	...
	test('count_documents returns zero when no rows are returned', async function () {
		...
		const query_builder = new QueryBuilder(create_model_stub(), 'count_documents', {}, null);
	...
	test('query builder chain methods configure find SQL and map rows', async function () {
		...
		const query_builder = new QueryBuilder(create_model_stub(), 'find', {active: true}, null);
	...
	test('find_one returns null when no rows are returned', async function () {
		...
		const query_builder = new QueryBuilder(create_model_stub(), 'find_one', {user_name: 'missing'}, null);
	...
	test('find_one defaults limit to 1 and hydrates rows through model.hydrate', async function () {
		const hydrated_document = {hydrated: true};
		const hydrate = jest.fn().mockReturnValue(hydrated_document);
		const connection = {
	...
	test('find_one always uses limit 1 even when a larger explicit limit is set', async function () {
		sql_runner_mock.mockResolvedValueOnce({
			rows: [{
				id: '7',
				data: {user_name: 'john'},
		...
		const query_builder = new QueryBuilder(create_model_stub(), 'find_one', {user_name: 'john'}, null);
	...
	test('find row fallback strips payload base fields and normalizes raw row metadata', async function () {
		...
		const query_builder = new QueryBuilder(create_model_stub(), 'find', {user_name: 'john'}, null);
	...
	test('throws for unsupported query operations', async function () {
		const query_builder = new QueryBuilder(create_model_stub(), 'aggregate', {}, null);

		await expect(query_builder.exec()).rejects.toThrow('Unsupported query operation: aggregate');
	});
});
