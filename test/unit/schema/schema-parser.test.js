...
describe('Schema field-definition parsing lifecycle', function () {
	test('defaults an omitted schema definition to an empty parsed schema', function () {
		expect(field_definition_parser()).toEqual({
			field_types: Object.create(null),
			object_paths: new Set()
		});
	});

	test('parses implicit, explicit, collection, and mixed field definitions', function () {
		const parsed_schema = field_definition_parser({
			title: String,
			age: {type: Number, min: 18},
			tags: [String],
			typed_list: {type: Array},
			typed_numbers: {type: Array, of: Number},
		...
		expect(parsed_schema.field_types.typed_list.instance).toBe('Array');
		expect(parsed_schema.field_types.typed_list.of_field_type.instance).toBe('Mixed');
		expect(parsed_schema.field_types.typed_numbers.instance).toBe('Array');
		expect(parsed_schema.field_types.typed_numbers.of_field_type.instance).toBe('Number');
	...
	test('parses nested object paths when a branch does not declare an explicit type', function () {
		const parsed_schema = field_definition_parser({
			profile: {
				nickname: String
			}
	...
	test('rejects invalid roots, invalid leaf definitions, and invalid array arity', function () {
	...
	test('parses unions and map of-definitions across supported shapes', function () {
		const parsed_schema = field_definition_parser({
			choice: {
				type: 'Union',
				of: [
	...
	test('rejects unsupported explicit, union, and collection of-definitions', function () {
		const registry_stub = {
			has_field_type(value) {
