...
describe('pool-store', function () {
	...
	describe('default state', function () {
		test('starts disconnected and exposes default state', function () {
			...
			expect(function get_connection_without_connect() {
				get_connection();
			}).toThrow('jsonbadger connection is not initialized. Call connect() first.');
		});
	});

	describe('set_pool', function () {
		test('stores pool state, connection instance, merges options, and snapshots frozen server capabilities', function () {
			const pool_instance = {tag: 'pool'};
			const connection_instance = {tag: 'connection'};
			...
			expect(get_connection()).toBe(connection_instance);
		...
		test('returns a cloned connection_options object on each call', function () {
			set_pool({
				pool_instance: {tag: 'pool'},
				options: {max: 3, debug: true}
			});
		...
		test('isolates nested connection options on write and read boundaries', function () {
			...
			const first_options_read = get_connection_options();
			first_options_read.ssl.rejectUnauthorized = true;
		...
		test('falls back to default connection options when options are omitted', function () {
			...
			expect(function get_connection_without_connection_instance() {
				get_connection();
			}).toThrow('jsonbadger connection is not initialized. Call connect() first.');
		});
	});

	describe('clear_pool', function () {
		test('resets pool, options, debug mode, and server capabilities', function () {
			set_pool({
				pool_instance: {tag: 'pool'},
				options: {debug: true, max: 2},
				server_capabilities: {
					server_version: '18.1',
					server_version_num: 180001,
					supports_uuidv7: true
			...
			expect(function get_connection_after_clear() {
				get_connection();
			}).toThrow('jsonbadger connection is not initialized. Call connect() first.');
		});
	});
