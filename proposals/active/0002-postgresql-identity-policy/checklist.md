...
- [ ] T1. Replace `src/constants/id-strategy.js` with `src/constants/identity-type.js`, `src/constants/identity-format.js`, and `src/constants/identity-mode.js`.
- [ ] T2. Update public exports so callers use `IDENTITY_TYPE`, `IDENTITY_FORMAT`, and `IDENTITY_MODE`.
- [ ] T3. Update `src/constants/defaults.js` so `defaults.schema_options.identity` is the single identity config namespace with:
  - [ ] T3A. `type = bigint`
  - [ ] T3B. `format = null`
  - [ ] T3C. `mode = fallback`
  - [ ] T3D. `generator = null`
