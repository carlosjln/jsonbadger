...
- [x] T2. Keep capability detection in `src/connection/connect.js`; create `Connection` with `server_capabilities` and do not add any extra runtime-selection object there.
- [x] T3. Update `src/schema/schema.js` so `schema.identity` remains declarative and runtime artifacts live under `schema.$runtime`.

- [x] T4. Add `Schema.prototype.$bind_connection(connection)` in `src/schema/schema.js`.
- [x] T5. Allow `schema.$bind_connection(null)` for connection-independent binding and fail clearly for capability-dependent paths.
- [x] T6. Update `src/model/factory/index.js` to clone the source schema, call `cloned_schema.$bind_connection(connection)`, and assign the bound clone to `model.schema`.

- [x] T7. Split JSONPath implementations into native and compat files under `src/sql/jsonb/read/operators/`.
- [x] T8. Bind JSONPath operator implementations into `schema.$runtime.read_operators` inside `Schema.prototype.$bind_connection(connection)`.
